    ...
    def collab_user_filter(self, userID):
        ...
        for hit in all_hits:
            for movieid in hit['_source']['ratings']:
                collab_movie_ids.append(movieid)

        return sorted(list(np.unique(collab_movie_ids)))
    ...
    def collab_movie_filter(self, movieId):
        ...
        for hit in all_hits:
            for user_id in hit['_source']['whoRated']:
                collab_user_ids.append(user_id)

        return sorted(list(np.unique(collab_user_ids)))


if __name__ == "__main__":
    ec = ElasticClient()
    print("User filter for no 78")
    print(ec.collab_user_filter(75))
    print("Movie filter for no 2")
    print(ec.collab_movie_filter(3))
